...
bool parseScenarioLocal(const char* filename, ScenarioData* data) {
    XMLDocument doc;
    ...
    for (XMLElement* lanelet = root->FirstChildElement("lanelet");
        ...
        data->lanePolys.push_back(buildLanePolygon(lane));
    }
    ...
    for (XMLElement* dynObs = root->FirstChildElement("staticObstacle");
        ...
        if (XMLElement* init = dynObs->FirstChildElement("initialState")) {
            ...
            data->trajectories[id].push_back(os);
        }
    }
...
void workerForObstacleLocal(int Id,std::atomic<int>& violationsCount,std::atomic<int>& collisionCount,const ScenarioData& data){
    const auto& myTraj = data.trajectories.at(Id);

    std::vector<int> otherIds;
    otherIds.reserve(data.trajectories.size() - 1);
    for (const auto& kv : data.trajectories) {
    ...
    for (const ObstacleState& s : myTraj) {
        ...
        if (!isInsideRoad(s,data.lanePolys) && s.t != 0) {
            //std::lock_guard<std::mutex> lock(printMutex);
        ...
        for (int oid : otherIds) {
            ...
            const  std::vector<ObstacleState>& oTraj = data.trajectories.at(oid);
...
void workerForScenario(const char* filename) {
    ScenarioData data;
    auto startTime = std::chrono::high_resolution_clock::now();
    if (!parseScenarioLocal(filename, &data)) {
        std::cerr << "Could not parse " << filename << "\n";
        return;
    }
    auto parseEndTime = std::chrono::high_resolution_clock::now();
    std::atomic<int> violationsCount{0};
    std::atomic<int> collisonCount{0};
    std::vector<std::thread> threads;
    threads.reserve(data.trajectories.size());
    for (const auto& kv : data.trajectories) 
        threads.emplace_back(workerForObstacleLocal, kv.first,
                     std::ref(violationsCount),
                     std::ref(collisonCount),
                     std::cref(data));
    ...
    globalResults.push_back({
        filename,
        std::chrono::duration<long double, std::milli>(parseEndTime - startTime).count(),
        std::chrono::duration<long double, std::milli>(endTime - parseEndTime).count(),
        violationsCount.load(),
        collisonCount.load(),
        (int)data.trajectories.size()
    });
    resultsMutex.unlock();
}
...
int main() {
    ...
    // pass by value or const std::string&; easiest: pass by value
    for (const auto& f : files)
    threads.emplace_back([](std::string fname){ workerForScenario(fname.c_str()); }, f);
    ...
    for (auto& f : globalResults) {
        ScenarioResult& r = f;
        totalParse     += r.parseMs;
        totalCompute   += r.computeMs;
